...
export default function StatsPage() {
  ...
  if (loading) return (
    <div className="flex-1 flex items-center justify-center bg-[#0A0A0A]">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-white/40 text-[10px] uppercase tracking-widest font-mono">Loading active telemetry...</p>
      </div>
    </div>
  );
  ...
  if (!stats) return (
    <div className="flex-1 p-8 text-center bg-[#0A0A0A] flex flex-col items-center justify-center">
      ...
      <p className="text-white/50 text-[10px] uppercase tracking-widest">Failed to load system statistics.</p>
      <button onClick={() => window.location.reload()} className="mt-4 text-xs text-amber-500 hover:text-amber-400 border border-amber-500/30 bg-amber-500/5 px-4 py-2 rounded-sm outline-none focus:border-amber-500">
        Retry Connection
      </button>
    </div>
  );
      ...
      <div className="w-full max-w-5xl px-6 py-12 md:py-20 relative">
        ...
        <header className="mb-16 border-b border-white/5 pb-8 relative z-10">
          <div className="inline-flex items-center gap-2 bg-[#121212] border border-white/10 rounded-full px-3 py-1 mb-6">
            ...
            <span className="text-[10px] uppercase tracking-[0.2em] text-white/50 font-semibold">System Operational</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-serif font-bold text-white mb-4 tracking-tight">Telemetry</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16 relative z-10">
          <div className="glass-panel p-6 rounded-sm border-l-2 border-l-amber-500">
            <p className="text-[10px] uppercase tracking-[0.2em] text-white/40 mb-2">Total Records</p>
            <p className="text-3xl font-mono font-bold text-white mb-1">{stats.totalDocuments ?? 0}</p>
            <p className="text-xs text-amber-500 flex items-center gap-1"><span className="material-symbols-outlined text-[12px]">trending_up</span> Live updating</p>
          </div>
          <div className="glass-panel p-6 rounded-sm border border-white/5">
            <p className="text-[10px] uppercase tracking-[0.2em] text-white/40 mb-2">Avg Search Time</p>
            <p className="text-3xl font-mono font-bold text-white mb-1">
              {(stats.averageSearchLatency || 0).toFixed(1)} <span className="text-sm text-white/30 font-sans">ms</span>
            </p>
            <p className="text-xs text-white/30">Target: &lt;50ms</p>
          </div>
          <div className="glass-panel p-6 rounded-sm border border-white/5">
            <p className="text-[10px] uppercase tracking-[0.2em] text-white/40 mb-2">Total Queries</p>
            <p className="text-3xl font-mono font-bold text-white mb-1">{stats.totalQueries ?? 0}</p>
          ...
          <div className="glass-panel p-6 rounded-sm border border-white/5">
            ...
            <p className="text-3xl font-mono font-bold text-white mb-1">
              {stats.uptime ? (stats.uptime / 3600).toFixed(1) : "0.0"} <span className="text-sm text-white/30 font-sans">hrs</span>
            </p>
          ...
          <div>
            ...
            <div className="glass-panel rounded-sm">
              ...
              <ul className="max-h-[300px] overflow-y-auto custom-scrollbar p-2">
                {stats.documentsBySubreddit && Object.entries(stats.documentsBySubreddit).map(([sub, count]: any) => {
                  const perc = stats.totalDocuments > 0 ? ((count / stats.totalDocuments) * 100).toFixed(1) : "0.0";
                  return (
                    <li key={sub}>
                      <button onClick={() => navigate(`/search?q=${sub}`)} className="w-full text-left p-3 flex gap-4 items-center hover:bg-[#1A1A1A] rounded-sm transition-colors group">
                        <div className="w-1/2 flex items-center gap-2">
                          <span className="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-amber-500 transition-colors" />
                          <span className="text-sm font-medium text-white/80 group-hover:text-white">r/{sub}</span>
                        </div>
                        <div className="w-1/4 text-right font-mono text-sm text-white/60">{count}</div>
                        <div className="w-1/4 text-right font-mono text-sm text-amber-500/70">{perc}%</div>
                      </button>
                    </li>
                  )
                })}
            ...
            <div className="flex flex-wrap gap-3">
              {stats.popularQueries && stats.popularQueries.length > 0 ? (
                stats.popularQueries.map((item: any) => (
                  ...
                  >
                    <span className="text-lg text-white group-hover:text-amber-500 font-serif font-semibold">{item.query}</span>
                    <span className="text-[10px] font-mono text-white/30 bg-white/5 px-1.5 py-0.5 rounded-sm">{item.count}</span>
                  </button>
                ))
              ) : (
                <div className="w-full py-12 text-center bg-[#121212] border border-white/5 rounded-sm border-dashed">
                  <span className="material-symbols-outlined text-white/10 text-3xl mb-2 block">hourglass_empty</span>
