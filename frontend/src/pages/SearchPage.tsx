  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row bg-[#0A0A0A]">
      ...
      <aside className="w-full md:w-64 shrink-0 bg-[#0A0A0A] border-r border-white/5 p-6 md:h-[calc(100vh-73px)] md:sticky md:top-[73px] overflow-y-auto hidden md:block custom-scrollbar">
        <div className="mb-10">
          <h3 className="text-[10px] uppercase tracking-[0.2em] text-white/30 font-semibold mb-4">
            View Mode
          </h3>
        ...
        <div className="mb-10">
          ...
          <div className="space-y-2">
            {['programming', 'javascript', 'python', 'machinelearning', 'devops', 'webdev'].map(
              (topic) => (
                <label key={topic} className="flex items-center gap-3 group cursor-pointer">
                  <input
                    type="checkbox"
                    defaultChecked={query.toLowerCase().includes(topic) || query === ''}
                    className="accent-[#864535] w-4 h-4 bg-transparent border-white/20 rounded-sm"
                  />
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {['Posts', 'Comments', 'Media'].map((type) => (
              <button
                key={type}
                className="px-3 py-1 bg-[#121212] border border-white/10 text-xs text-white/50 rounded-sm hover:border-white/30 hover:text-white transition-colors"
              >
                {type}
              </button>
            ))}
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              <div className="py-20 text-center bg-[#1D1D1D] rounded-sm premium-card">
                <span className="material-symbols-outlined text-4xl text-white/10 mb-4 block">
                  search_off
                </span>
                <p className="text-white/50 text-sm mb-6">
                  No records found for <strong className="text-white">"{query}"</strong>
                </p>
                  ...
                  <div className="flex items-start gap-6">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="text-[10px] font-bold text-[#864535] uppercase tracking-widest border border-[#864535]/20 px-2 py-0.5 rounded-sm bg-[#864535]/5">
                          r/{result.metadata?.subreddit || 'Unknown'}
                        </span>
                        <span className="text-[10px] text-white/40 font-mono">
                          u/{result.metadata?.author || 'archivist'}
                        </span>
                      </div>
                        ...
                        dangerouslySetInnerHTML={{
                          __html:
                            result.snippet?.replace(
                              /\*\*(.*?)\*\*/g,
                              '<strong class="text-white bg-white/10 px-1 rounded-sm">$1</strong>'
                            ) || '',
                        }}
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-3 shrink-0">
                      <div className="flex items-center gap-2 bg-[#121212] border border-white/5 px-2 py-1 rounded-sm">
                        <span className="material-symbols-outlined text-[14px] text-[#864535]">
                          thumb_up
                        </span>
                        <span className="text-xs text-white font-mono">
                          {result.metadata?.redditScore || 0}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 bg-[#121212] border border-white/5 px-2 py-1 rounded-sm">
          ...
          {results.length > 0 && (
            <div className="mt-12 pt-8 border-t border-white/5 flex justify-between items-center">
              <p className="text-xs text-white/40 font-mono uppercase tracking-widest">
                Page 1 of {Math.ceil(metadata.totalCount / 20)}
              </p>
