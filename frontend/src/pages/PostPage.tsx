    ...
    <div className="flex-1 bg-[#0A0A0A] flex flex-col relative">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-6 py-12 md:py-20 relative z-10">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-8">
          <Link
            to={`/search?q=${doc.subreddit}`}
            className="bg-amber-500/10 text-amber-500 border border-amber-500/20 text-[10px] uppercase tracking-widest px-3 py-1 font-bold rounded-sm hover:bg-amber-500 hover:text-[#0A0A0A] transition-colors"
          >
            r/{doc.subreddit}
          </Link>
          ...
          <span className="text-white/40 text-xs font-mono">
            {doc.createdUtc
              ? new Date(doc.createdUtc).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                })
        ...
        <div className="flex flex-wrap items-center gap-6 py-4 border-y border-white/10 mb-12 bg-[#121212]/50 px-4 rounded-sm">
          <div className="flex items-center gap-2 text-white/70">
            ...
            <span className="text-sm font-semibold font-mono">
              {(doc.redditScore || 0).toLocaleString()}
            </span>
          </div>
          <div className="flex items-center gap-2 text-white/70">
            <span className="material-symbols-outlined text-[16px]">chat_bubble</span>
            <span className="text-sm font-semibold font-mono">
              {(doc.commentCount || 0).toLocaleString()}
            </span>
          </div>
          ...
          <button className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-white/40 hover:text-white transition-colors group">
            <span className="material-symbols-outlined text-[16px] group-hover:text-amber-500 transition-colors">
              bookmark
            </span>{' '}
            Save
          </button>
